...
var SignallingServer = function(room, socketServer){
    ...
    //When receive "full" signal from the signal server
    this.socket.on('full', function (room){
      trace('Room ' + room + ' is full');
      this.onRoomFull(room);
    }.bind(this));

    //When receive "empty" signal from the signal server
    this.socket.on('empty', function (room){
      this.isInitiator = true;
      trace('Room ' + room + ' is empty');
    });

    //When receive "join" signal from the signal server
    this.socket.on('join', function (room){
      trace('Making request to join room ' + room);
    });

    //When receive "joined" signal from the signal server
    ...
    //When receive "sdp received" signal from the signal server
